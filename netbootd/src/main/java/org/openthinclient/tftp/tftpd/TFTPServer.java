...
public class TFTPServer implements Runnable {
  ...
  private static final String DEFAULT_BOOTFILE = "/ipxe.kpxe";
  ...
  private class TFTPSend extends Thread {
    ...
    private void parseRequest(SocketAddress peer, ByteBuffer buffer,
      ...
      // Thinkpad addon to catch the right pxe file.
      if (filename.startsWith(bootfile)){
    	  filename = bootfile;
      }
  ...
  public TFTPServer() throws IOException {
    this(null, DEFAULT_TFTP_PORT, DEFAULT_BOOTFILE);
  }
  ...
  public TFTPServer(int port) throws IOException {
    this(null, port, DEFAULT_BOOTFILE);
  }
  ...
  public TFTPServer(String basedir) throws IOException {
    this(basedir, DEFAULT_TFTP_PORT, DEFAULT_BOOTFILE);
  }
   ...
   * @param basedir the root directory
   * @param port the port to use
   * @param bootfile the file default to boot from (iPXE bootloader)
   * @throws IOException
   */
  public TFTPServer(String basedir, int port, String bootfile) throws IOException {
    if (null != basedir){
